...
public class Test2 {
  ...
  public static void main(String[] args) {
    ...
    IIslandShape islandShape = IslandShapeFactory.createIslandShape(islandShapeType, random);
    ...
    for (Point point : points) {
      ...
      if (isInside(islandShape, size, point)) {
        result += "L";
      } else {
        result += "W";
      }
  ...
  private static boolean isInside(IIslandShape islandShape, int size, Point point) {
    return islandShape.isInside(new Point(2 * (point.x / size - 0.5), 2 * (point.y / size - 0.5)));
  }
}
